# Tiny CNN VeRi baseline with tqdm + VeRi protocol (DirectML-ready) + CIs for CMC & Precision
# Outputs:
#   - veri_cnn_results.json
#   - outputs/learning_curve_cnn.png
#   - outputs/cnn_calibration.png
#   - figs/Good_example.jpg, figs/mixed_example.jpg, figs/bad_example.jpg
#
...
# -----------------------
# CONFIG (shared shape with SVM.py)
# -----------------------
CONFIG = {
    "DATA_ROOT": r"C:\Users\cecil\Documents\dataset\archive\VeRi",
    "IMG_SIZE":  [128, 128],  # CNN input (H, W)
    "SEED": 42,
    "OUT_DIR": "outputs",
    ...
    "LR": 1e-3,
    "WEIGHT_DECAY": 5e-4,
    "AMP": False,  # AMP only on CUDA; DirectML does not support AMP
}
...
# -----------------------
# Helpers (mirrored style with SVM.py)
# -----------------------
...
# -----------------------
# Evaluation (VeRi protocol) — with per-query AP, CMC indicators, and Precision@k
# -----------------------
def eval_veri_verbose(q_emb, q_names, g_emb, g_names, gt_map, jk_map, topk=(1,5,10)):
    """
    Returns:
      metrics (means),
      ap_per_q (Q,),
    ...
    for qi in tqdm(range(Q.shape[0]), desc="eval: VeRi (AP/CMC/Precision)", unit="q"):
        ...
        # Precision@k (fraction of true among first k after junk removal)
        for kidx, k in enumerate(topk):
...
# -----------------------
# Qualitative montage (query + top-5)
# -----------------------
...
def get_device_and_amp():
    ...
    if torch.cuda.is_available():
        ...
        use_amp = bool(CONFIG["AMP"])  # AMP only on CUDA
    elif dml_device is not None:
        device = dml_device           # DirectML device
        use_amp = False               # no AMP on DirectML
    else:
...
def main():
    ...
    scaler = torch.cuda.amp.GradScaler(enabled=use_amp)  # only meaningful on CUDA
    ...
    topk = (1,5,10)
    metrics, ap_list, cmc_ind, prec_at_k, order = eval_veri_verbose(
        q_emb[q_idx], q_ref, g_emb[g_idx], g_ref, gt_map, jk_map, topk=topk
    )

    # CIs (mAP, CMC@k, Precision@k, FP@k)
    alpha = 1.0 - CONFIG["CI"]/100.0
    ci = bootstrap_ci_multi(
    ...
    # Qualitative strips: good / mixed / bad
    def ranked_kept_names(qi):
        ranked = order[qi]
        keep_mask = np.array([g_ref[j] not in jk_map.get(qi, set()) for j in ranked], dtype=bool)
        return [g_ref[j] for j in ranked[keep_mask]]

    figs_dir = Path("figs"); figs_dir.mkdir(exist_ok=True)
    q_dir, g_dir = root / "image_query", root / "image_test"
    gt_sets = [gt_map.get(i, set()) for i in range(len(q_ref))]
    good_idx = mixed_idx = bad_idx = None
    for qi in range(len(q_ref)):
        names = ranked_kept_names(qi)
        is_good = np.array([n in gt_sets[qi] for n in names], dtype=bool)
        top10 = is_good[:10]
        if good_idx is None and top10[:1].any(): good_idx = qi
        if mixed_idx is None and (top10.any() and (~top10[:10]).any()): mixed_idx = qi
        if bad_idx is None and not top10.any(): bad_idx = qi
        if good_idx is not None and mixed_idx is not None and bad_idx is not None: break

    if good_idx is not None:
        save_strip(figs_dir/"Good_example.jpg", q_dir, g_dir, q_ref[good_idx], ranked_kept_names(good_idx), k=5)
    if mixed_idx is not None:
        save_strip(figs_dir/"mixed_example.jpg", q_dir, g_dir, q_ref[mixed_idx], ranked_kept_names(mixed_idx), k=5)
    if bad_idx is not None:
        save_strip(figs_dir/"bad_example.jpg", q_dir, g_dir, q_ref[bad_idx], ranked_kept_names(bad_idx), k=5)
...
# -----------------------
# Bootstrap helper (legacy for mAP-only; not used anymore by main)
# -----------------------
def bootstrap_ci(values, n_boot=1000, ci=95):
    vals = np.array(values, dtype=np.float64)
    n = len(vals)
    if n == 0: return 0.0, (0.0, 0.0)
    means = [np.mean(np.random.choice(vals, size=n, replace=True)) for _ in range(n_boot)]
